...
Ithil supports almost any ERC20 tokens and offers a stable yield for liquidity providers willing to lend their assets to traders. The protocol takes a fixed and a percentual fee on borrowed assets and redistributes it to governance token holders and a small percentage to the developers behind it. We make strategies simple and intuitive for any trader to take part in: via a set of smart contracts and several investment strategies, Ithil help users maximise their rewards from various liquidity pools (LPs), automated market makers (AMM), yield aggregators, staking pools and other income bearing opportunities in the DeFi ecosystem. This way we provide a huge advantage over attempting to do this manually yourself and risking losing all your assets. -->

## Undercollateralised loans

As described above, the transfer from the vault to the various strategies on behalf of a trader can be considered an undercollateralised loan, in which the amount borrowed is higher than the margin posted by the user. This is possible since the assets taken as loan are locked into the strategy, they do not go outside Ithil and cannot get into user possession.

### Ithil's Vault and fees

The lending part is the **Vault**, where **liquidity providers** can stake their tokens and get the fees generated by the traders running investments. The list of supported, or **whitelisted**, tokens is managed by the DAO following security and market criteria to prevent possible scams. The vault can technically support any ERC20 token, including meme coins, taxed, rebasing and reflecting tokens, thus making Ithil a good choice to get APY in virtually *any token*.

INSERIRE QUI IMMAGINE VAULT

The liquidity given to the strategy in the form of loans is backed by the user's margin and by independent **liquidators**, which close the positions whose margin has been eroded by more than a strategy and token-dependant **risk factor** and get rewarded for offering this service. Since, due to very unfavorable market conditions, liquidators may fail to properly liquidate a given position, the liquidity of the vault is further protected via the **insurance reserve**: liquidity set aside and used to repay stakers' losses due to bad liquidations. The amount of the insurance reserve is adjusted *algorithmically* to ensure good capital protection and, at the same time, a competitive interest rate for liquidity providers.
...
The final interest rate is then directly proportional to these four numbers, i.e. it's the multiplication of all of them renormalised by a global factor. The interest rate is computed considering the vault state at the time of opening a position, while the due fees are calculated at the closure of the same position.
...
In order to assure a fair and gas-efficient fees distribution method, Ithil adopts the classical method of yield-bearing tokens, which are minted and sent to liquidity providers in a quantity reflecting the *share price* of the staked LP tokens. The **total wealth** of the vault is the whole quantity of tokens deposited by liquidity providers, plus all fees generated and not yet claimed. This value can be calculated as the total balance excluding governance-owned liquidity, plus the open loans. When a liquidity provider deposits some amount of TKA, then some amount of iTKA will be minted to the LP, calculated as follows.

INSERIRE QUI IMMAGINE CALCOLO MINTED AMOUNT

Similarly, an holder of iTKA can burn at any time its i-tokens, thus receiving a transfer of TKA calculated as follows.
...
In this way, a liquidity provider trying to deposit and withdraw immediately some funds will receive exactly the same amount deposited and only the fees generated *after* the deposit will contribute to the provider's APY: no arbitrage is possible. Moreover, a simple calculation shows that neither deposits nor withdrawals change the share price, hence the effect of a liquidity provider withdrawing or depositing is not affecting the others.
...
Besides calling the relevant external contracts to perform the particular investment, the specific strategies must also implement a **quoter**, a view function which gives at every time the value of the obtained assets with respect to the initial token used to obtain them. The quoter is important for liquidations and cross-margin investments (INSERIRE HYPERLINK).
